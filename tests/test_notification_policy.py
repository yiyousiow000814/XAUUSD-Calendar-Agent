...
def test_notification_policy_suppresses_same_message_even_when_state_changes() -> None:
    previous = MarketState(
        current_bias="neutral",
        main_driver="technical_liquidation",
    ...
    assert decision.should_notify is False
    assert decision.reason == "Same alert message remains inside notification cooldown."


def test_notification_policy_suppresses_unconfirmed_churn_inside_cooldown() -> None:
    previous = MarketState(
        current_bias="bearish_gold",
        main_driver="yields",
        secondary_driver="usd",
    ...
    result = replace(
        load_builtin_fixture("unconfirmed_move").expected_rule_based_result,
        should_notify=True,
    ...
    assert decision.should_notify is False
    assert decision.reason == "Unconfirmed alert remains inside notification cooldown."
